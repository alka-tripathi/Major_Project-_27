  ...
  const fetchDoctorProfile = async (currentUser: any) => {
    try {
      ...
      if (data.success && data.doctor) {
        ...
        if (data.doctor.name) {
          const parts = data.doctor.name.split(" ");
          docFirstName = parts[0];
          docLastName = parts.slice(1).join(" ");
        }

        setFormData({
          firstName: docFirstName || initialFirstName,
          lastName: docLastName || initialLastName,
